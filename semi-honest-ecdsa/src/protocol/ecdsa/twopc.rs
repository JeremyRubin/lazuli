...
pub fn run<T: 'static>(m: &[u64; 4], mut peer: T) -> Option<secp256k1::Signature>
where
    ...
    let r = {
        ...
        if leader {
            ...
            let kkG = {
                ...
                let mut kG = secp256k1::PublicKey::from_slice(ctx, &b33).ok()?;
                kG.mul_assign(ctx, &b32_nonce);
                kG
            };
            peer.write_all(&kkG.serialize()[1..]).ok()?;
            peer.flush().ok()?;
